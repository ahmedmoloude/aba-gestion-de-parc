...
export interface ServiceAmountsSum {
  [service: string]: { ht: number; ttc: number };
}
@Component({
  selector: 'app-affretement-global-facture',
  templateUrl: './affretement-global-facture.component.html',
  styleUrls: ['./affretement-global-facture.component.css']
})
export class AffretementGlobalFactureComponent implements OnInit, OnDestroy {
  ...
  constructor(private store:Store<AppState>,
    ...
    public dialogRef: MatDialogRef<AffretementGlobalFactureComponent>,
    ...
    this.billSubscription = this.bill$.subscribe(
      (resp) => {
        if (resp?.dataState == 'Success' && resp?.facture ) {
          this.amountsSumByService = this.calculateAmountsSumByService(resp.facture?.demandes);
        }
      }
    );
  }
