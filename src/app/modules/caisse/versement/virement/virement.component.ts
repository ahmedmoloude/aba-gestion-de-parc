...
import { loadVersementsVirement, validateVersementVirement } from 'app/core/store/caisse/versement/versement.actions';
...
export class VirementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.versement$.subscribe(
      (resp: VersementState) => {
        if(resp.virementDataState == StateEnum.SUCCESS )
        this.versements = resp.versementsVirement.filter(v => v.remis != "REMIS");
      });
  ...
  filtrer($event){
    ...
    this.store.dispatch(loadVersementsVirement({data: versement}))
  }
  ...
  validate(versement?: Versement) {
    ...
    this.store.dispatch(validateVersementVirement({data:{documents: documents}}));
    this.selection.clear();
  }
