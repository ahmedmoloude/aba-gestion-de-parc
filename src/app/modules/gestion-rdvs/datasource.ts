export let scheduleData: Object[] = [
  {
    Id: 1,
    Subject: 'Explosion of Betelgeuse Star',
    StartTime: new Date(2022, 1, 11, 9, 30),
    EndTime: new Date(2022, 1, 11, 11, 0),
    CategoryColor: '#1aaa55',
  },
  {
    Id: 2,
    Subject: 'Thule Air Crash Report',
    StartTime: new Date(2022, 1, 12, 12, 0),
    EndTime: new Date(2022, 1, 12, 14, 0),
    CategoryColor: '#357cd2',
  },
  {
    Id: 3,
    Subject: 'Blue Moon Eclipse',
    StartTime: new Date(2022, 1, 13, 9, 30),
    EndTime: new Date(2022, 1, 13, 11, 0),
    CategoryColor: '#7fa900',
  },
  {
    Id: 4,
    Subject: 'Meteor Showers in 2022',
    StartTime: new Date(2022, 1, 14, 13, 0),
    EndTime: new Date(2022, 1, 14, 14, 30),
    CategoryColor: '#ea7a57',
  },
  {
    Id: 5,
    Subject: 'Milky Way as Melting pot',
    StartTime: new Date(2022, 1, 15, 12, 0),
    EndTime: new Date(2022, 1, 15, 14, 0),
    CategoryColor: '#00bdae',
  },
  {
    Id: 6,
    Subject: 'Mysteries of Bermuda Triangle',
    StartTime: new Date(2022, 1, 15, 9, 30),
    EndTime: new Date(2022, 1, 15, 11, 0),
    CategoryColor: '#f57f17',
  },
  {
    Id: 7,
    Subject: 'Glaciers and Snowflakes',
    StartTime: new Date(2022, 1, 16, 11, 0),
    EndTime: new Date(2022, 1, 16, 12, 30),
    CategoryColor: '#1aaa55',
  },
  {
    Id: 8,
    Subject: 'Life on Mars',
    StartTime: new Date(2022, 1, 17, 9, 0),
    EndTime: new Date(2022, 1, 17, 10, 0),
    CategoryColor: '#357cd2',
  },
  {
    Id: 9,
    Subject: 'Alien Civilization',
    StartTime: new Date(2022, 1, 19, 11, 0),
    EndTime: new Date(2022, 1, 19, 13, 0),
    CategoryColor: '#7fa900',
  },
  {
    Id: 10,
    Subject: 'Wildlife Galleries',
    StartTime: new Date(2022, 1, 21, 11, 0),
    EndTime: new Date(2022, 1, 21, 13, 0),
    CategoryColor: '#ea7a57',
  },
  {
    Id: 11,
    Subject: 'Best Photography 2022',
    StartTime: new Date(2022, 1, 22, 9, 30),
    EndTime: new Date(2022, 1, 22, 11, 0),
    CategoryColor: '#00bdae',
  },
  {
    Id: 12,
    Subject: 'Smarter Puppies',
    StartTime: new Date(2022, 1, 9, 10, 0),
    EndTime: new Date(2022, 1, 9, 11, 30),
    CategoryColor: '#f57f17',
  },
  {
    Id: 13,
    Subject: 'Myths of Andromeda Galaxy',
    StartTime: new Date(2022, 1, 7, 10, 30),
    EndTime: new Date(2022, 1, 7, 12, 30),
    CategoryColor: '#1aaa55',
  },
  {
    Id: 14,
    Subject: 'Aliens vs Humans',
    StartTime: new Date(2022, 1, 5, 10, 0),
    EndTime: new Date(2022, 1, 5, 11, 30),
    CategoryColor: '#357cd2',
  },
  {
    Id: 15,
    Subject: 'Facts of Humming Birds',
    StartTime: new Date(2022, 1, 20, 9, 30),
    EndTime: new Date(2022, 1, 20, 11, 0),
    CategoryColor: '#7fa900',
  },
  {
    Id: 16,
    Subject: 'Sky Gazers',
    StartTime: new Date(2022, 1, 23, 11, 0),
    EndTime: new Date(2022, 1, 23, 13, 0),
    CategoryColor: '#ea7a57',
  },
  {
    Id: 17,
    Subject: 'The Cycle of Seasons',
    StartTime: new Date(2022, 1, 12, 5, 30),
    EndTime: new Date(2022, 1, 12, 7, 30),
    CategoryColor: '#00bdae',
  },
  {
    Id: 18,
    Subject: 'Space Galaxies and Planets',
    StartTime: new Date(2022, 1, 12, 17, 0),
    EndTime: new Date(2022, 1, 12, 18, 30),
    CategoryColor: '#f57f17',
  },
  {
    Id: 19,
    Subject: 'Lifecycle of Bumblebee',
    StartTime: new Date(2022, 1, 15, 6, 0),
    EndTime: new Date(2022, 1, 15, 7, 30),
    CategoryColor: '#7fa900',
  },
  {
    Id: 20,
    Subject: 'Sky Gazers',
    StartTime: new Date(2022, 1, 15, 16, 0),
    EndTime: new Date(2022, 1, 15, 18, 0),
    CategoryColor: '#ea7a57',
  },
];
export let eventsData: Object[] = [
  {
    Id: 1,
    Subject: 'Server Maintenance',
    StartTime: new Date(2022, 1, 11, 10, 0),
    EndTime: new Date(2022, 1, 11, 11, 30),
    EventType: 'maintenance',
    City: 'Seattle',
    CategoryColor: '#1aaa55',
  },
  {
    Id: 2,
    Subject: 'Art & Painting Gallery',
    StartTime: new Date(2022, 1, 12, 12, 0),
    EndTime: new Date(2022, 1, 12, 14, 0),
    EventType: 'public-event',
    City: 'Costa Rica',
    CategoryColor: '#357cd2',
  },
  {
    Id: 3,
    Subject: 'Dany Birthday Celebration',
    StartTime: new Date(2022, 1, 13, 10, 0),
    EndTime: new Date(2022, 1, 13, 11, 30),
    EventType: 'family-event',
    City: 'Kirkland',
    CategoryColor: '#7fa900',
  },
  {
    Id: 4,
    Subject: 'John Wedding Anniversary',
    StartTime: new Date(2022, 1, 14, 9, 0),
    EndTime: new Date(2022, 1, 14, 10, 0),
    EventType: 'family-event',
    City: 'Redmond',
    CategoryColor: '#ea7a57',
  },
  {
    Id: 5,
    Subject: 'ISA Annual Conference',
    StartTime: new Date(2022, 1, 15, 10, 0),
    EndTime: new Date(2022, 1, 15, 11, 30),
    EventType: 'commercial-event',
    City: 'USA',
    CategoryColor: '#00bdae',
  },
  {
    Id: 6,
    Subject: 'Equipment Maintenance',
    StartTime: new Date(2022, 1, 16, 12, 0),
    EndTime: new Date(2022, 1, 16, 14, 0),
    EventType: 'maintenance',
    City: 'Seattle',
    CategoryColor: '#f57f17',
  },
  {
    Id: 7,
    Subject: 'Aircraft Maintenance',
    StartTime: new Date(2022, 1, 17, 10, 0),
    EndTime: new Date(2022, 1, 17, 11, 30),
    EventType: 'maintenance',
    City: 'Seattle',
    CategoryColor: '#1aaa55',
  },
  {
    Id: 8,
    Subject: 'Facilities Maintenance',
    StartTime: new Date(2022, 1, 19, 9, 30),
    EndTime: new Date(2022, 1, 19, 11, 0),
    EventType: 'maintenance',
    City: 'Seattle',
    CategoryColor: '#357cd2',
  },
  {
    Id: 9,
    Subject: 'Britto Birthday Celebration',
    StartTime: new Date(2022, 1, 21, 11, 0),
    EndTime: new Date(2022, 1, 21, 13, 0),
    EventType: 'family-event',
    City: 'Greenland',
    CategoryColor: '#7fa900',
  },
  {
    Id: 10,
    Subject: 'Justin Wedding Anniversary',
    StartTime: new Date(2022, 1, 22, 9, 30),
    EndTime: new Date(2022, 1, 22, 11, 0),
    EventType: 'family-event',
    City: 'Finland',
    CategoryColor: '#ea7a57',
  },
  {
    Id: 11,
    Subject: 'AIEA Annual Meet',
    StartTime: new Date(2022, 1, 9, 10, 0),
    EndTime: new Date(2022, 1, 9, 11, 30),
    EventType: 'commercial-event',
    City: 'USA',
    CategoryColor: '#00bdae',
  },
  {
    Id: 12,
    Subject: 'AAN Conference',
    StartTime: new Date(2022, 1, 7, 10, 30),
    EndTime: new Date(2022, 1, 7, 12, 30),
    EventType: 'commercial-event',
    City: 'USA',
    CategoryColor: '#f57f17',
  },
  {
    Id: 13,
    Subject: 'Photography Gallery',
    StartTime: new Date(2022, 1, 5, 10, 0),
    EndTime: new Date(2022, 1, 5, 11, 30),
    EventType: 'public-event',
    City: 'Chennai',
    CategoryColor: '#1aaa55',
  },
  {
    Id: 14,
    Subject: 'Beach Clean-up',
    StartTime: new Date(2022, 1, 14, 12, 0),
    EndTime: new Date(2022, 1, 14, 2, 0),
    EventType: 'public-event',
    City: 'Mumbai',
    CategoryColor: '#357cd2',
  },
  {
    Id: 15,
    Subject: 'Turtle Walk',
    StartTime: new Date(2022, 1, 19, 13, 0),
    EndTime: new Date(2022, 1, 19, 14, 30),
    EventType: 'public-event',
    City: 'Costa Rica',
    CategoryColor: '#7fa900',
  },
  {
    Id: 16,
    Subject: 'Silent Walk for Cancer day',
    StartTime: new Date(2022, 1, 22, 13, 0),
    EndTime: new Date(2022, 1, 22, 14, 30),
    EventType: 'public-event',
    City: 'Chennai',
    CategoryColor: '#ea7a57',
  },
];
export let eventData: Object[] = [
  {
    Id: 1,
    Subject: 'Surgery - Andrew',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 12, 9, 0),
    EndTime: new Date(2022, 1, 12, 10, 0),
  },
  {
    Id: 2,
    Subject: 'Consulting - John',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 12, 10, 0),
    EndTime: new Date(2022, 1, 12, 11, 30),
  },
  {
    Id: 3,
    Subject: 'Therapy - Robert',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 12, 11, 30),
    EndTime: new Date(2022, 1, 12, 12, 30),
  },
  {
    Id: 4,
    Subject: 'Observation - Steven',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 12, 12, 30),
    EndTime: new Date(2022, 1, 12, 13, 30),
  },
  {
    Id: 5,
    Subject: 'Extraction - Nancy',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 12, 13, 30),
    EndTime: new Date(2022, 1, 12, 15, 0),
  },
  {
    Id: 6,
    Subject: 'Surgery - Paul',
    EventType: 'New',
    StartTime: new Date(2022, 1, 13, 9, 0),
    EndTime: new Date(2022, 1, 13, 10, 0),
  },
  {
    Id: 7,
    Subject: 'Extraction - Josephs',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 13, 10, 0),
    EndTime: new Date(2022, 1, 13, 11, 0),
  },
  {
    Id: 8,
    Subject: 'Consulting - Mario',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 13, 11, 0),
    EndTime: new Date(2022, 1, 13, 12, 0),
  },
  {
    Id: 9,
    Subject: 'Therapy - Saveley',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 13, 12, 0),
    EndTime: new Date(2022, 1, 13, 13, 30),
  },
  {
    Id: 10,
    Subject: 'Observation - Cartrain',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 13, 13, 30),
    EndTime: new Date(2022, 1, 13, 15, 30),
  },
  {
    Id: 11,
    Subject: 'Consulting - Yang',
    EventType: 'New',
    StartTime: new Date(2022, 1, 14, 9, 0),
    EndTime: new Date(2022, 1, 14, 10, 0),
  },
  {
    Id: 12,
    Subject: 'Observation - Michael',
    EventType: 'New',
    StartTime: new Date(2022, 1, 14, 10, 0),
    EndTime: new Date(2022, 1, 14, 11, 30),
  },
  {
    Id: 13,
    Subject: 'Surgery - Roland',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 14, 11, 30),
    EndTime: new Date(2022, 1, 14, 12, 30),
  },
  {
    Id: 14,
    Subject: 'Extraction - Francisco',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 14, 12, 30),
    EndTime: new Date(2022, 1, 14, 13, 30),
  },
  {
    Id: 15,
    Subject: 'Therapy - Henriette',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 14, 13, 30),
    EndTime: new Date(2022, 1, 14, 15, 0),
  },
  {
    Id: 16,
    Subject: 'Observation - Bernardo',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 15, 9, 0),
    EndTime: new Date(2022, 1, 15, 10, 0),
  },
  {
    Id: 17,
    Subject: 'Therapy - Wilson',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 15, 10, 0),
    EndTime: new Date(2022, 1, 15, 11, 0),
  },
  {
    Id: 18,
    Subject: 'Consulting - Horst',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 15, 11, 0),
    EndTime: new Date(2022, 1, 15, 12, 0),
  },
  {
    Id: 19,
    Subject: 'Surgery - Limeira',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 15, 12, 0),
    EndTime: new Date(2022, 1, 15, 13, 30),
  },
  {
    Id: 20,
    Subject: 'Observation - Victoria',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 15, 13, 30),
    EndTime: new Date(2022, 1, 15, 15, 30),
  },
  {
    Id: 21,
    Subject: 'Extraction - Afonso',
    EventType: 'Confirmed',
    StartTime: new Date(2022, 1, 16, 9, 0),
    EndTime: new Date(2022, 1, 16, 10, 0),
  },
  {
    Id: 22,
    Subject: 'Extraction - Paula',
    EventType: 'New',
    StartTime: new Date(2022, 1, 16, 10, 0),
    EndTime: new Date(2022, 1, 16, 11, 0),
  },
  {
    Id: 23,
    Subject: 'Observation - George',
    EventType: 'Requested',
    StartTime: new Date(2022, 1, 16, 11, 0),
    EndTime: new Date(2022, 1, 16, 12, 0),
  },
  {
    Id: 24,
    Subject: 'Therapy - Smith',
    EventType: 'New',
    StartTime: new Date(2022, 1, 16, 12, 0),
    EndTime: new Date(2022, 1, 16, 13, 30),
  },
  {
    Id: 25,
    Subject: 'Surgery - Jennifer',
    EventType: 'New',
    StartTime: new Date(2022, 1, 16, 13, 30),
    EndTime: new Date(2022, 1, 16, 15, 30),
  },
];
